...
public class HospitalService {
  ...
  public HospitalLog registraLog(Hospital hospital) {
    return this.hospitalLogRepository.save(
        HospitalLog.builder()
            ...
            .capacidade(hospital.getCapacidade())
            .build());
  }
  ...
  public Hospital alterarNrDePacientes(Long id, Long pacientes) {
    ...
    if (hospital == null) {
      throw new HospitalNaoEncontradoException(
          "Hospital não encontrado. Não foi possível alterar número de pacientes.");
    }
  ...
  public void registraLogDeRecursos(Hospital h1, Hospital h2, Recurso r1, Recurso r2) {
    this.recursoLogRepository.save(
        RecursoLog.builder()
            .dataHora(LocalDateTime.now())
            .total(r1.soma(r2))
            .enviadoHospital(h1)
  ...
  public boolean realizaTransacaoEntreHospitais(
      Long h1, Long h2, Recurso aReceber, Recurso aEnviar) {
  ...
  public Double porcentagemDeHospitaisComOcupacaoMaiorQue90Porcento() {
    ...
    Long nrHospitaisComMaisDe90Porcento =
        hospitais.stream()
            .filter(
                (hospital ->
                    (hospital.getPacientes().doubleValue() / hospital.getCapacidade().doubleValue())
                        >= 0.9d))
            .count();
  ...
  public Double porcentagemDeHospitaisComOcupacaoMenorQue90Porcento() {
    ...
    Long nrHospitaisComMenosDe90Porcento =
        hospitais.stream()
            .filter(
                (hospital ->
                    (hospital.getPacientes().doubleValue() / hospital.getCapacidade().doubleValue())
  ...
  public Map<String, Double> mediasDeRecursos() {
    ...
    OptionalDouble optAmbulancia =
        hospitais.stream().mapToLong(value -> value.getRecursos().getAmbulancia()).average();
    OptionalDouble optEnfermeiro =
        hospitais.stream().mapToLong(value -> value.getRecursos().getEnfermeiro()).average();
    OptionalDouble optMedico =
        hospitais.stream().mapToLong(value -> value.getRecursos().getMedico()).average();
    OptionalDouble optRespirador =
        hospitais.stream().mapToLong(value -> value.getRecursos().getRespirador()).average();
    OptionalDouble optTomografo =
        hospitais.stream().mapToLong(value -> value.getRecursos().getTomografo()).average();

    Double ambulancia =
        optAmbulancia.orElseThrow(
            () -> new RuntimeException("Não pode realizar a média de ambulancias."));
    Double enfermeiro =
        optEnfermeiro.orElseThrow(
            () -> new RuntimeException("Não pode realizar a média de enfermeiros."));
    Double medicos =
        optMedico.orElseThrow(() -> new RuntimeException("Não pode realizar a média de médicos."));
    Double respirador =
        optRespirador.orElseThrow(
            () -> new RuntimeException("Não pode realizar a média de respiradores."));
    Double tomografo =
        optTomografo.orElseThrow(
            () -> new RuntimeException("Não pode realizar a média de tomógrafos."));
  ...
  public Hospital superLotacaoAMaisTempo() {
    ...
    Map<Long, Long> mediaLotacaoPorHospital =
        hospitalLogs.stream()
            .filter(hospitalLog -> hospitalLog.lotacao() >= 0.9)
            .collect(
                Collectors.groupingBy(
                    hospitalLog -> hospitalLog.getHospital().getId(),
                    Collectors.reducing(
                        0l,
                        value -> value.getDataHora().toEpochSecond(ZoneOffset.UTC),
                        Long::sum)));

    Map.Entry<Long, Long> entry =
        mediaLotacaoPorHospital.entrySet().stream()
            .max(Comparator.comparing(Map.Entry::getValue))
            .get();

    return this.hospitalPorId(entry.getKey());
  }

  public Hospital baixaLotacaoAMaisTempo() {
    List<HospitalLog> hospitalLogs = this.listaLogHospital();

    List<HospitalLog> filter =
        hospitalLogs.stream()
            .filter(hospitalLog -> hospitalLog.lotacao() < 0.9)
        ...
        filter.stream()
            .collect(
                Collectors.groupingBy(
                    hospitalLog -> hospitalLog.getHospital().getId(),
                    Collectors.reducing(
                        0l, o -> o.getDataHora().toEpochSecond(ZoneOffset.UTC), Long::sum)));
